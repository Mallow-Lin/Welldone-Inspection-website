import {
  faCalendarCheck,
  faHardHat,
  faFileSignature,
  faIdCard,
} from '@fortawesome/free-solid-svg-icons'
import type { ServiceItem } from './services'

const otherServices: ServiceItem[] = [
  {
    title: "Engineer's Report",
    name: "Engineer's Report",
    shortDescription: 'Evaluation and Assessment',
    longDescription: 'Evaluation and Assessment',
    background: '/images/services/engineers-report.jpg',
    link: '',
  },
  {
    title: 'TPP',
    name: 'TPP',
    shortDescription: 'TPP Filing and Weekly Inspection',
    longDescription: 'TPP Filing and Weekly Inspection',
    icon: faCalendarCheck,
    link: '',
  },
  {
    title: 'Site Safety Plan',
    name: 'Site Safety Plan',
    shortDescription: 'Site Safety Plan, DOT Plan, Superintendent, SSC, SSM',
    longDescription: 'Site Safety Plan, DOT Plan, Superintendent, SSC, SSM',
    icon: faHardHat,
    link: '',
  },
  {
    title: 'Expediting',
    name: 'Expediting',
    shortDescription: 'DOB Filing, DOB Violations, ECB Violations',
    longDescription: 'DOB Filing, DOB Violations, ECB Violations',
    icon: faFileSignature,
    link: '',
  },
  {
    title: 'Shop Drawings',
    name: 'Shop Drawings',
    shortDescription: 'MEP Shop Drawings, Structural Shop Drawings, Architectural Shop Drawings',
    longDescription: 'MEP Shop Drawings, Structural Shop Drawings, Architectural Shop Drawings',
    background: '/images/services/shop-drawings.webp',
    link: '',
  },
  {
    title: 'Licensed Contractor',
    name: 'Licensed Contractor',
    shortDescription: 'Permit Application: GC, SOE, FO, ST, MEP',
    longDescription: 'Permit Application: GC, SOE, FO, ST, MEP',
    icon: faIdCard,
    link: '',
  },
]

export default otherServices
